...
| G5 | 身份模型 | 主体只有一种：**账户（account）**；凭据有两种：**企微 JWT** + **`sk-xxxx` 静态 key**。同一 token 通用于 LLM 与 MCP 调用 |
| G6 | 两层归因 | **暂不做**（服务账户代调用时，只算调用方账户的账，不追溯背后员工） |
...
| G12 | 号池与模型分配 | **逻辑模型方案（方案 B）+ 三层模型**：模型分配（账户→可用模型）/ 模型目录（逻辑模型→多承载渠道）/ 号池（渠道→多 key）。多上游与多 key 均坍缩为 litellm.Router 的 deployment，轮换/熔断由 Router 白送 |
...
> **开源网关的前端对本项目零复用价值**：任何开源网关都自带它自己的一套管理 UI，覆盖不了本项目要的全部管理模块（号池/模型目录/模型分配/账户/配额/MCP 工具统计），风格 / 权限 / 登录也是两套。管理后台前端无论如何都得自研，所以「前端」不构成 build/buy 的选型差异。
...
> 解决文档此前的缺口：公司在各厂商的 API key 如何统一管理（号池），以及管理后台如何灵活控制「哪个账户能打到哪些模型」（模型分配）。采用**逻辑模型方案（方案 B）**：员工看到统一模型名，背后可由多个上游承载。

### 三层职责（不可压成一层）

```
① 模型分配层 —— 账户/部门 能用哪些逻辑模型 + 默认模型
        │ （对员工的访问控制）
        ▼
...
account_model_grant     模型分配 = 账户/部门 能用哪些逻辑模型
  id, scope(account|department), scope_id
  logical_model_id→logical_model
  is_default(该 scope 的默认模型)
  注：账户级覆盖部门级；裁决时取并集，account 优先
```

> 配额/用量表按 `account × logical_model × 日` 记（完整 schema 见第八章 4）。号池的健康/冷却状态运行时落 Redis，DB 存配置与审计。

### 管理后台对应模块

- **号池管理**：渠道 CRUD、key 录入/轮换/启停、连通性测试、健康看板、`last_used` 僵尸 key 发现
- **模型目录**：逻辑模型 CRUD、承载渠道绑定（权重/主备）、计价配置
- **模型分配**：按部门批量分配 + 按账户覆盖、设默认模型、即时生效（Router 配置短 TTL 重载或热更新）
  ...
  class HermesTokenVerifier(TokenVerifier):
      async def verify_token(self, token: str) -> AccessToken | None:
          ...
          if principal is None:
              return None
          return AccessToken(token=token, client_id=principal.account_id,
...
- MCP 工具内部若需要 LLM 生成能力，**直接 HTTP 调平台自己的 LLM 网关**（内部调用，同账户身份、配额、记账）——**不走 MCP 的 sampling 协议**。
...
- **主体（principal）= 配额 / 账单算在谁头上**：只有一种 —— **账户**。
- **凭据（credential）= 拿什么证明自己是这个账户**：两种。

| 凭据 | 主体 | 签发方式 | 用途 |
|---|---|---|---|
| 企微 JWT | 账户 | 扫码登录，2h 过期 + refresh | Hermes 桌面端 |
| `sk-xxxx` 静态 key | 账户 | 自助门户生成 | 脚本 / curl / OA 系统 / 任意程序化调用 |

**两种凭据都解析成同一个账户**，落到同一个配额桶。网关鉴权层分流：

```
Authorization: Bearer xxx
  ├─ 是 JWT（验签通过） → 账户 principal
  └─ 是 sk-xxxx → DB 哈希查表 → 账户 principal
之后：统一 principal → 一个配额桶 → 路由 / 记账（只写一遍）
```

### 账户的两种来源

- **真人账户**：员工，企微扫码登录，可在自助门户生成自己的 `sk-key`。
- **服务账户**：管理员创建的非真人账户（如 `oa-system`、`batch-jobs`），无人扫码，仅持有 `sk-key` 和配额。OA 系统、批处理等系统级调用方挂在服务账户名下。

> 一张账户表搞定所有 principal，**不需要独立的「应用注册」子系统**。这是相对早期设想的简化（去掉了「应用」作为独立主体类型）。
...
src/
  ...
  admin/       管理后台 API（号池/模型目录/模型分配/账户/配额）
...
| `dependencies.py` | FastAPI `Depends()` 注入项（取 DB session、鉴权、取当前账户等） |
...
nginx（反向代理 + 负载均衡 + TLS 终结）
  ...
  └─ 副本 N：全功能（按并发增减）
共享后端（已有现成实例，本项目不部署）：
  ├─ PostgreSQL（账户 / key / 模型分配 / 用量 / 审计；MCP 无专属表，复用账户/审计）
...
- **优先**：共享数据模型（第八章 4）—— 账户 / key / 模型分配 / 配额 / 用量 / 审计，把前面所有讨论落成 schema（MCP 模块无新表，复用以上）。

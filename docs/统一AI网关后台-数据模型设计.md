...
| 项 | 约定 | 说明 |
|---|---|---|
| 表名 | `snake_case`、**单数** | SQLAlchemy 社区惯例。`user` 而非 `users` |
...
class BaseEntity(Base):
    ...
    create_dept: Mapped[int|None] = mapped_column(BigInteger, nullable=True, index=True)  # 创建者当时所在部门,通用数据权限过滤用
...
- `created_by`/`updated_by` **不做硬外键，只存 `sys_user.id`（软引用）**：避免用户删除连带破坏历史；系统自动行为存 `null` 表「系统」。
- `create_dept` **不做硬外键，只存 `sys_department.id`（软引用）**：记录创建者当时所在部门，通用业务表按它承接 `dept` / `dept_and_child` 数据权限；`sys_user` 列表仍按用户自身 `department_id` 过滤。
...
> 解决「哪个用户/部门能打到哪些逻辑模型 + 默认模型」。早期草案曾按 account 命名，现已统一为 `sys_user` / `user_id`，定为 `user_model_grant`。

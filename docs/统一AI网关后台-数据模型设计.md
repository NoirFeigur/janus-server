...
| 项 | 约定 | 说明 |
|---|---|---|
| 表名 | `snake_case`、**单数** | SQLAlchemy 社区惯例。`user` 而非 `users`；`users` 用复数因 `user` 是 PG 保留字 |
...
**配置/业务表**继承统一基类 `BaseEntity`（审计 + 创建部门公共字段）：
...
class BaseEntity(Base):
    ...
    created_by: Mapped[int|None] = mapped_column(BigInteger, nullable=True)   # 操作者 users.id,软引用,系统行为 null
    create_dept: Mapped[int|None] = mapped_column(BigInteger, nullable=True, index=True)  # 创建者当时所在部门,仅作审计留痕
...
- `created_by`/`updated_by` **不做硬外键，只存 `users.id`（软引用）**：避免用户删除连带破坏历史；系统自动行为存 `null` 表「系统」。
- `create_dept` **不做硬外键，只存 `department.id`（软引用）**：记录创建者当时所在部门，仅作审计留痕；`users` 列表按用户自身 `department_id` 过滤。
...
# —— 通用状态（多表共用，避免每表重定义）——
class ActiveStatus(StrEnum):        # 配置表通用启停态:role/menu/logical_model/model_deployment/quota 等
    active = "active"               # 启用
...
# —— 第 1.5 批 RBAC ——
class MenuType(StrEnum):            # 权限资源节点类型(接口/菜单/按钮合一于 menu)
...
class ErrorCode(StrEnum):           # 业务错误码(problem+json 的 type/code);成员值即下发给前端的 code
    ...
    auth_user_disabled      = "auth.user_disabled"        # 用户已停用(离职/封禁)
...
> 注：`QuotaScope.global_` 必须带尾下划线——`global` 是 Python 关键字，不能直接做成员名；存库值仍是 `"global"`。其余枚举成员名 = 值。
...
```
第 1 批（基础主体与身份）
  users                用户基础档案（HR 同步，principal 真相源）
  department           部门树（邻接表）
  user_oauth           第三方登录身份（企微等）→ users
  api_key              平台签发的 sk-key 凭据 → users

第 1.5 批（管理后台 RBAC，标准企业级四权限）
  role                 角色
  menu                 菜单/按钮/接口权限统一表（menu_type 区分）
  user_role            用户↔角色（多对多）→ users + role
  role_menu            角色↔权限（多对多）→ role + menu
...
第 2 批（号池与模型三层，架构稿四之三）
  ...
  user_model_grant        模型分配：用户/部门能用哪些逻辑模型 → users + logical_model

第 3 批（配额与计量）
  usage_record            单次调用用量流水（per-call ledger）→ users + logical_model
  quota                   多层配额规则（user/department/global × 模型 × 周期 × 维度）

第 4 批（MCP 服务器，架构稿 G11/四之五）
  （无新表）          平台自身即标准 MCP 服务器,工具是平台内代码;鉴权复用 users/api_key(第1批),
...
> - **grant（`user_model_grant`）= AI 资源消费权限**——「全体员工能用哪些模型」，粒度细、与用户/部门强绑。MCP 工具的业务授权交下游第三方（不建 grant 表，G11/四之五）。
...
| 公共列 | 配置表 `BaseEntity`（id/is_deleted/created_by/create_dept/created_at/updated_by/updated_at）；流水表 `LogEntity`（id/created_at）；关联表 `LinkEntity`（id/created_at，物理删） |
...
| 表名 | 主体表 `users` 用复数（避 PG 保留字 `user`），其余表单数去 `sys_` 前缀；实体类名不变（如 `User`/`Role`） |
...
> - `users` —— 平台权威用户档案，真相源是 **HR 系统**（定时同步）。
> - `user_oauth` —— 第三方登录身份快照（企微等），真相源是 **第三方**。一个用户每 source 一个身份。
> - `department` —— 组织部门树，模型分配的 scope 之一（架构稿四之三）。
> - `api_key` —— 平台签发给用户的 sk-key 调用凭据，存哈希。
> 服务账号（`oa-system` 等）暂不做（本轮决定），将来需要时再单独设计，不污染 `users`。

### 表 1.1：`users`（用户基础档案，HR 同步）
...
class User(BaseEntity):
    ...
    department_id: Mapped[int|None] = mapped_column(BigInteger, nullable=True, index=True)  # → department,逻辑引用无 FK
...
要点：
- **`users` 只放基础档案**，不含任何企微/第三方字段（移到 1.3）。
...
### 表 1.3：`user_oauth`（第三方登录身份，精简自 JustAuth）

```python
class UserOAuth(BaseEntity):
    __tablename__ = "user_oauth"

    user_id:  Mapped[int]      = mapped_column(BigInteger, index=True)   # → users,NOT NULL(方案1 硬匹配命中才落库)
...
扫码拿到 `(source=wecom, uuid=企微UserID)` 后，若 `user_oauth` 无此身份，要绑定到某个 `users` 行。
...
扫码 → (source=wecom, uuid=企微UserID)
     → ① 查 users WHERE legacy_employee_no = uuid   # 老员工:企微 = 旧工号
          ↑ 仅匹配非空 legacy_employee_no(新员工此列 null,SQL 等值天然不命中 null,无需特判)
     → ② ① 未命中再查 WHERE employee_no = uuid          # 新人(无旧工号):企微 = 新工号
     → 命中:写 user_oauth(wecom, uuid) → user_id,完成绑定(此处才落 user_id)
...
- **`user_id` 仅在命中时写入**：未命中绝不建 `user_oauth` 行（呼应 1.3 表 `user_id NOT NULL`——不存在「待绑定 null」中间态）。
- **零模糊匹配逻辑**：不靠 email/手机这类可变、可重复的字段，直接工号硬匹配，最稳。
- 两个工号列在 1.1 均 `index=True` + 部分唯一索引，匹配走索引。

> **唯一需警惕的碰撞**：若**某新员工的新工号**恰好等于**某老员工的旧工号**（两套编号空间重叠），步骤①会先把该 uuid 绑到老员工身上（错绑）。
...
class ApiKey(BaseEntity):
    ...
    user_id:   Mapped[int]      = mapped_column(BigInteger, index=True)   # → users,key 归属人
...
- **`key_hash` 用 sha256 即可**（不用 bcrypt）：key 本身是高熵随机串（非用户低熵密码），无需抗暴力慢哈希；sha256 可建唯一索引 + 等值查验，每次请求快速命中。（对比 `users.password` 用 argon2/bcrypt——那是低熵人造密码，必须慢哈希。）
...
# user_oauth —— 通常不直接对外,内部绑定用;如需展示则只露非敏感
...
> 模型对标若依（RuoYi）：**接口/菜单/按钮三权限合一于 `menu`**（`menu_type` 区分，`perms` 串复用于前端按钮显隐 + 后端 API 鉴权）。
>
...
|---|---|---|---|---|
| 菜单/按钮/接口权限 | **合一** `menu`+`perms` | 拆 `blade_menu` + `blade_scope_api` | **学 RuoYi 合一** | 一个 `perms` 码前后端复用，避免权限定义漂移 |
| 用户↔部门 | 单 `department_id` | 多对多 `blade_user_dept` | **单部门**（决策 2026-06-15） | 经确认企微侧无大量多部门，单字段 99% 场景够用 |
| 角色层级 | 扁平 | 角色树 `parent_id` | **扁平**（决策 2026-06-15） | 后台用户十几人，多角色绑定已足；角色树要递归算权限，过度设计 |
| 用户↔角色 | `user_role` 关联表 | ⚠️ `blade_user.role_id` 存逗号 CSV 串 | **学 RuoYi 关联表** | BladeX CSV 串违反第一范式、无法 join/索引，是反模式 |
| 关联表主键 | 纯复合主键 | 代理 id `blade_role_menu.id` | **学 BladeX 代理 id**（= 本平台 `LinkEntity`） | 代理雪花 id + `UNIQUE` 防重，比纯复合主键多留 `created_at`（授权时间可审计） |

> **被 BladeX 反向验证的两处旧决策**：① `blade_role_menu` 用代理 id 而非复合主键 → 背书我们的 `LinkEntity`；② `blade_user_oauth` 同样无 `blog/company/location` → 背书我们删 oauth 冗余快照字段（见 1.3 差异表）。数据权限将来作为独立统一模块另行设计。

### 三种权限如何落到表（先理清，再看字段）

| 权限种类 | 落在哪 | 机制 |
|---|---|---|
| **菜单权限** | `menu`（`menu_type=catalog/menu`） | 控制前端路由/侧栏显隐 |
| **按钮权限** | `menu`（`menu_type=button` + `perms`） | 控制前端按钮显隐 |
...
class Role(BaseEntity):
    ...
    # 前端树勾选联动开关（纯 UI 行为,对 ry menu_check_strictly）
...
class UserRole(LinkEntity):
    ...
    user_id: Mapped[int] = mapped_column(BigInteger, index=True)   # → users
    role_id: Mapped[int] = mapped_column(BigInteger, index=True)   # → role
...
class RoleMenu(LinkEntity):
    ...
    role_id: Mapped[int] = mapped_column(BigInteger, index=True)   # → role
    menu_id: Mapped[int] = mapped_column(BigInteger, index=True)   # → menu
...
**理由**：给 `user_role` 套 `is_deleted` 反而麻烦——「取消某用户的角色」应是物理删一行，不是标记软删（软删后重新授权又撞唯一、查询全要带 `WHERE is_deleted`）。关联表物理删是业内常规，也与 RuoYi 参考库一致（其关联表无 `del_flag`）。

> **关于授权问责（已决策不做）**：Oracle 评审建议给 `LinkEntity` 加 `created_by` 或另立 `admin_audit_log` 记「谁把角色授给谁/谁撤销」。**本轮明确不做**——后台用户仅十几人、授权变更低频，问责审计的成本大于收益。`created_at` 留着（知道何时授权），「谁授的」暂不追。**将来若合规要求后台操作审计，再统一加 `admin_audit_log`（同理 `call_audit` 调用审计也是届时再加）**，届时是加表不是改这几张关联表的结构，迁移代价小。
...
表 2.5  user_model_grant    模型分配(谁能用哪些逻辑模型)    → users/department + logical_model
```
...
> 这把 key 是**上游厂商发给我们的**（要拿去调上游），所以**可逆加密存**（0.6 已定，与 `users.password`/`api_key.key_hash` 的单向哈希方向相反）。
...
- 两个加密方向对照（全平台密钥三态）：
  ...
  | `users.password` | 验证管理员密码(只比对) | **单向慢哈希** argon2/bcrypt |
...
你拍这个（weight 相乘 ✓/✗、priority 选 A/B），我锁 2.4 进本批最后一张 2.5 `user_model_grant`（模型分配：谁能用哪些逻辑模型 + 默认模型 + 用户/部门 scope）。
...
> 解决「哪个用户/部门能打到哪些逻辑模型 + 默认模型」。早期草案曾按 account 命名，现已统一为 `users` / `user_id`，定为 `user_model_grant`。
...
class UserModelGrant(BaseEntity):
    ...
    scope_id: Mapped[int] = mapped_column(BigInteger, index=True)  # scope=user→users.id; scope=department→department.id
...
用户张三请求 logical_model = "claude-sonnet":
  有效授权集 = user_model_grant where (scope=user, scope_id=张三.id)        # 用户级
             ∪ user_model_grant where (scope=department, scope_id=张三.department_id)  # 部门级继承
  ① X ∈ 有效授权集的 logical_model 集 ？  → 是则放行,否则 403
  ② 员工没传 model(用默认):
       默认模型 = 用户级 is_default 优先;无则取部门级 is_default
       (用户级覆盖部门级 —— 架构稿已定)
```

- **取并集 + 用户级优先**：用户能用的 = 自己被授的 ∪ 所在部门被授的；默认模型用户级覆盖部门级。
- **部门级授权批量省事**：给「技术部」授 `claude-sonnet`，全部门自动可用，不用逐人配。用户级用于个别覆盖（如给某人额外开一个模型）。
...
| `user_model_grant` | 模型分配(AI 消费权限,B 轴) | 多态 scope；用户级覆盖部门级；is_default 部分唯一 |
...
class UsageRecord(LogEntity):
    ...
    # —— 主体(算在谁头上)——
    user_id:    Mapped[int]      = mapped_column(BigInteger, index=True)   # → users,principal
